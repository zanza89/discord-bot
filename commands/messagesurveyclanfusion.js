...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('messagesurveyclanfusion')
	...
	async execute(interaction) {
		...
		interaction.guild.members.fetch().then((members) => {
			...
			members.forEach((member) => {
				...
				member.send({ content: '', ephemeral: true, embeds: [embedOffi], components: [actionrow] }).catch(console.error);
				member.send({ content: '', ephemeral: true, embeds: [embedFusion], components: [actionrow2] }).catch(console.error);
			});
		});
